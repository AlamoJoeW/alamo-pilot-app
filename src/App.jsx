...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (info) { setPilot(info); loadFromCache() }
  }, [])
  ...
  async function sync() {
    ...
    try {
      ...
      setSites(data.sites); setSyncedAt(data.syncedAt)
      await flushPending()
    } catch (err) {
  ...
  const handleUpdate = useCallback(async (recordId, action) => {
    const changes = { collectedApp: action === 'collected', partialCollection: action === 'partial', mobFee: action === 'mob' }
    if (action === 'uncollect') { changes.collectedApp = false; changes.partialCollection = false; changes.mobFee = false }
    ...
    // If uncollecting, also remove from EOD queue
    if (action === 'uncollect') {
    ...
    } else { await queueUpdate({ recordId, action }); setPendingCount(n => n + 1) }
  ...
  const handleEodToggle = useCallback((recordId) => {
    ...
    if (selectedSite?.id === recordId) {
      setSelectedSite(prev => prev ? { ...prev, _eodPending: !prev._eodPending } : prev)
    }
  }, [selectedSite])
  ...
  function handleLogout() { logout(); clearAll(); setPilot(null); setSites([]); setSelectedSite(null); setEodQueue(new Set()) }
  function handleLogin(data) { const info = getPilotInfo(); setPilot(info || data); sync() }
    ...
    <div className="app">
      <header className="top-bar">
        ...
        <div className="top-bar-right">
          ...
          <button className="icon-btn" onClick={() => setShowChangePassword(true)} title="Change password">
            PW
          </button>
          <button className="icon-btn" onClick={handleLogout} title="Log out">Out</button>
        </div>
      </header>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            {eodSummary && eodSummary.error ? (
              ...
              <div>
                ...
                <p className="eod-note">EOD report submitted to Airtable.</p>
              </div>
            ) : <p>Submitting...</p>}
