  ...
  return (
        <div className="modal-overlay" onClick={onClose}>
                <div className="modal" onClick={e => e.stopPropagation()}>
                          <h2>Change Password</h2>h2>
                  {success ? (
                    <>
                                <p style={{ color: '#22c55e', textAlign: 'center', marginBottom: 20 }}>Password updated successfully</p>p>
                                <button className="btn-primary" onClick={onClose}>Done</button>button>
                    </>>
                  ) : (
                    <form onSubmit={handleSubmit}>
                      ...
                      {error && <p style={{ color: '#ef4444', marginBottom: 12, fontSize: 14 }}>{error}</p>p>}
                                <div style={{ display: 'flex', gap: 10 }}>
                                              <button type="button" className="btn-secondary" onClick={onClose} style={{ flex: 1 }}>Cancel</button>button>
                                              <button type="submit" className="btn-primary" disabled={submitting} style={{ flex: 1 }}>{submitting ? 'Saving...' : 'Save Password'}</button>button>
                                </div>div>
                    </form>form>
                        )}
                </div>div>
