 ...
 *
 * Props:
 *   pilot      â pilot info object from auth (name, etc.)
 *   onComplete â callback(preflightId) called after successful POST
 */
...
// âââ Inline field components âââââââââââââââââââââââââââââââââââââââââââââââââââ
...
export default function Preflight({ pilot, onComplete }) {
  ...
  function handleNext() {
    setError('')
    if (stepIndex < steps.length - 1) { setStepIndex(i => i + 1); window.scrollTo(0, 0) }
  }

  function handleBack() {
    setError('')
    if (stepIndex > 0) { setStepIndex(i => i - 1); window.scrollTo(0, 0) }
  }
  ...
  if (loading) return <div className="preflight-screen"><div className="pf-loading">Loading preflightâ¦</div></div>

  return (
    <div className="preflight-screen">
      <div className="pf-header">
        <div className="pf-header-title">
          <span className="pf-icon">âï¸</span>
          <div>
            <h1 className="pf-title">Daily Preflight</h1>
            <p className="pf-subtitle">
              {pilot?.firstName || pilot?.displayName || 'Pilot'} Â· {new Date().toLocaleDateString()}
            </p>
          </div>
        </div>
        <div className="pf-progress-row">
          ...
          <div className="pf-dots">
            {steps.map((_, i) => (
              <div key={i} className={`pf-dot ${i < stepIndex ? 'done' : i === stepIndex ? 'active' : ''}`} />
            ))}
          </div>
          <span className="pf-step-name">{stepLabel(currentStep)}</span>
        </div>
        <div className="pf-progress-track">
          <div className="pf-progress-fill" style={{ width: `${((stepIndex + 1) / totalSteps) * 100}%` }} />
        </div>
      </div>

      <div className="pf-body">
        {error && <div className="error-msg" style={{ marginBottom: 16 }}>{error}</div>}
        {currentStep === 1 && <StepBasicInfo form={form} projects={projects} aircraft={aircraft} onProjectChange={handleProjectChange} onAircraftChange={v => setField('aircraftId', v)} onTravelDayChange={handleTravelDayToggle} />}
        {currentStep === 2 && <StepTravelDay form={form} onChange={v => setField('travelingTo', v)} />}
        {currentStep === 3 && <StepFlightInfo form={form} projectType={projectType} setField={setField} />}
        {currentStep === 4 && <StepImsafeWeather form={form} setField={setField} />}
        {currentStep === 5 && <StepAirworthinessAirspace form={form} setField={setField} />}
        {currentStep === 6 && <StepRiskAssessment form={form} setField={setField} />}
        {currentStep === 7 && <StepFinal form={form} setField={setField} />}
      </div>

      <div className="pf-nav">
        {stepIndex > 0 && <button className="pf-btn-back" onClick={handleBack} disabled={submitting}>â Back</button>}
        {currentStep === 2 || stepIndex === steps.length - 1 ? (
          <button className="btn-primary btn-full" onClick={handleSubmit} disabled={submitting}>
            {submitting ? 'Submittingâ¦' : 'Submit Preflight'}
          </button>
        ) : (
          <button className="btn-primary pf-btn-next" onClick={handleNext}>Next â</button>
        )}
      </div>
    </div>
  )
...
function StepBasicInfo({ form, projects, aircraft, onProjectChange, onAircraftChange, onTravelDayChange }) {
  return (
    <div className="pf-step">
      ...
      <SelectField label="Project" value={form.projectId} onChange={onProjectChange} options={projects.map(p => ({ value: p.id, label: p.name }))} />
      <SelectField label="Aircraft" value={form.aircraftId} onChange={onAircraftChange} options={aircraft.map(a => ({ value: a.id, label: a.name }))} />
...
function StepTravelDay({ form, onChange }) {
  return (
    <div className="pf-step">
      ...
      <p className="pf-step-desc">No flight operations today. Enter your destination and submit.</p>
...
function StepFlightInfo({ form, projectType, setField }) {
  return (
    <div className="pf-step">
      ...
      {projectType === 'methane' && <SelectField label="Basin" value={form.basin} onChange={v => setField('basin', v)} options={BASIN_OPTIONS} />}
...
function StepAirworthinessAirspace({ form, setField }) {
  return (
    <div className="pf-step">
      ...
      {form.airworthy === 'No' && <TextAreaField label="Airworthiness Notes" value={form.airworthyNotes} onChange={v => setField('airworthyNotes', v)} placeholder="Describe the issueâ¦" rows={3} />}
...
function StepRiskAssessment({ form, setField }) {
  return (
    <div className="pf-step">
      <h2 className="pf-step-heading">Risk* ssessment</h2>
      ...
      <SelectField label="Temperature (Â°F)" value={form.wxTemp} onChange={v => setField('wxTemp', v)} options={WX_TEMP_OPTIONS} />
...
function StepFinal({ form, setField }) {
  return (
    <div className="pf-step">
      ...
      <TextAreaField label="Additional Risks" value={form.additionalRisks} onChange={v => setField('additionalRisks', v)} placeholder="Any other risks identifiedâ¦" rows={3} />
