 ...
 *   onComplete – callback(preflightId) called after successful POST
 */
...
function buildSteps(projectType, travelDay) {
  // If travel day, only step 1 (basic) + travel shortcut step
  if (travelDay) return [1, 2]
  const steps = [1, 3, 4, 5]
  // Risk matrix — skip for verizon
...
export default function Preflight({ pilot, onComplete }) {
  ...
  // Capture GPS on mount (non-blocking)
  useEffect(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => { /* GPS unavailable — submission will proceed without it */ }
      )
    }
  }, [])

  // Load projects + aircraft on mount
  useEffect(() => {
  ...
  // When travel day toggles, reset step index to 0
  function handleTravelDayToggle(val) {
    setField('travelDay', val)
    setStepIndex(0)
  }

  // When project changes, reset step index to avoid landing on a stale step
    ...
    <div className="preflight-screen">
      ...
      <div className="pf-body">
        ...
        {currentStep === 2 && (
          <StepTravelDay
            form={form}
            onChange={v => setField('travelingTo', v)}
          />
        )}
      ...
      <div className="pf-nav">
        ...
        {/* Travel day shortcut submit on step 2, otherwise Next / Submit */}
...
function StepBasicInfo({ form, projects, aircraft, onProjectChange, onAircraftChange, onTravelDayChange }) {
  ...
  return (
    <div className="pf-step">
      ...
      />
      <CheckboxField
        label="Travel Day (no flight operations today)"
        checked={form.travelDay}
        onChange={onTravelDayChange}
      />
    </div>
  )
}

// ─── Step 2: Travel Day Shortcut ───────────────────────────────────────────────
  ...
  return (
    <div className="pf-step">
      ...
      <p className="pf-step-desc">
        No flight operations today. Enter your destination and submit to log this travel day.
      </p>
      <InputField
        label="Traveling To?"
  ...
  return (
    <div className="pf-step">
      ...
      <CheckboxField
        ...
        checked={form.visualObserver}
        onChange={v => setField('visualObserver', v)}
      />
      <InputField
        label="Acres Assigned"
        value={form.acres}
        onChange={v => setField('acres', v)}
        placeholder="e.g. 150"
        type="number"
      />
      {projectType === 'methane' && (
        <SelectField
          label="Basin"
          value={form.basin}
          onChange={v => setField('basin', v)}
          options={BASIN_OPTIONS}
        />
      )}
      <InputField
        label="Closest Flight Address"
        value={form.closestFlightAddr}
        onChange={v => setField('closestFlightAddr', v)}
        placeholder="Street address near flight area"
      />
      <InputField
  ...
  return (
    <div className="pf-step">
      ...
      <MultiSelectField
        ...
        value={form.imsafe}
        onChange={v => setField('imsafe', v)}
        options={IMSAFE_OPTIONS}
      />
      ...
      <MultiSelectField
        ...
        value={form.weatherCheck}
        onChange={v => setField('weatherCheck', v)}
        options={WEATHER_CHECK_OPTIONS}
      />

      <TextAreaField
        label="Weather Forecast Notes"
        value={form.weatherForecast}
        onChange={v => setField('weatherForecast', v)}
        placeholder="Describe forecast, wind, visibility, etc."
        rows={3}
      />
    </div>
  )
...
function StepAirworthinessAirspace({ form, setField }) {
  return (
    <div className="pf-step">
      ...
      <SelectField
        label="Aircraft Airworthy?"
        value={form.airworthy}
        onChange={v => setField('airworthy', v)}
        options={['Yes', 'No']}
      />
      {form.airworthy === 'No' && (
        <TextAreaField
          label="Airworthiness Notes"
          value={form.airworthyNotes}
          onChange={v => setField('airworthyNotes', v)}
      ...
      <MultiSelectField
        ...
        value={form.airspace}
        onChange={v => setField('airspace', v)}
        options={AIRSPACE_OPTIONS}
      />

      <SelectField
        label="TFR / NOTAM Present?"
        value={form.tfrPresent}
        onChange={v => setField('tfrPresent', v)}
        options={['Yes', 'No']}
      />

      <SelectField
        label="LAANC Required?"
        value={form.laancRequired}
        onChange={v => setField('laancRequired', v)}
        options={['Yes', 'No']}
      />
    </div>
  )
  ...
  return (
    <div className="pf-step">
      ...
      <SelectField
        ...
        value={form.riskTotal}
        onChange={v => setField('riskTotal', v)}
        options={RISK_TOTAL_OPTIONS}
      />
    </div>
  )
}
  ...
  return (
    <div className="pf-step">
      ...
      <TextAreaField
        ...
        value={form.additionalRisks}
        onChange={v => setField('additionalRisks', v)}
        placeholder="Any other risks identified…"
        rows={3}
      />
      ...
      <TextAreaField
        ...
        onChange={v => setField('mitigatingFactors', v)}
        placeholder="Steps taken to mitigate risks…"
        rows={3}
      />

      <CheckboxField
        label="Mission Overview reviewed and understood"
        checked={form.missionOverview}
        onChange={v => setField('missionOverview', v)}
      />

      <SelectField
        label="GO / NO-GO"
        value={form.goNogo}
        onChange={v => setField('goNogo', v)}
        options={['GO', 'NO-GO']}
      />
      ...
      <TextAreaField
        ...
        placeholder="Anything else to note for today…"
        rows={3}
      />
    </div>
  )
