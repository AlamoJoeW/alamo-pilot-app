...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Pull every site (no per-pilot filter — that's the only difference from /api/sites)
    const records = []
    let offset = null
    do {
    ...
    // Pull pilots once to build an id -> display name lookup for the pilotApp linked field
    const pilotNameById = {}
    {
      let pOffset = null
      do {
        ...
        if (pOffset) qs.set('offset', pOffset)
