...
export default async function handler(req, res) {
  ...
  // POST — create a single EOD record linking all collected/partial/mob sites
  if (req.method === 'POST') {
    try {
      const pilot = verifyToken(req)
      const today = new Date().toISOString().split('T')[0]
      ...
      // Airtable v0 linked-record fields expect plain string arrays, NOT { id } objects
      const fields = {
        [FIELDS.EOD_DATE]: today,
        [FIELDS.EOD_PILOT]: [pilot.pilotRecordId],
      }
      if (collectedIds.length > 0) {
        fields[FIELDS.EOD_FULL_COLLECTION] = collectedIds.map(String)
      }
      if (partialIds.length > 0) {
        fields[FIELDS.EOD_PARTIAL_COLLECTION] = partialIds.map(String)
      }
      if (mobIds.length > 0) {
        fields[FIELDS.EOD_MOBILIZATION] = mobIds.map(String)
      }

      // One POST = one EOD record containing all sites
